    ...
    <form class="form-inline">
      <div class="form-group">
        <div class="input-group">
          ...
          <input ref="src" class="input" type="text" placeholder="图标src"/>
        </div>
      </div>
      <div class="form-group">
        <div class="input-group">
          <div class="input-group-addon"><i class="glyphicon glyphicon-road"></i></div>
          <input ref="path" class="input" type="text" placeholder="社交链接"/>
        </div>
      </div>
      <div class="form-group">
  ...
  methods: {
    remove (index) {
      ...
      this.$modal({
        ...
        text: '正在删除是否继续？',
        sureFn: function () {
          vm.$store.dispatch('removeSocailContact', index)
        }
      })
    },
    ...
    save (index) {
      ...
      if (this.isUpdate) {
        ...
        if (!updateData.data.src || !updateData.data.path) {
          return this.$toast({text: '内容最好不要为空!', type: 'e'})
        }
        this.$store.dispatch('updateSocailContact', updateData)
        .then(() => {
          this.$toast({text: '修改成功!', type: 's'})
          this.buttonText = '添加'
          this.isUpdate = false
          this.$refs.src.value = ''
        ...
        if (!data.src || !data.path) {
          return this.$toast({text: '保存内容不能空!', type: 'e'})
        }
        this.$store.dispatch('saveSocailContact', data)
        .then(() => {
          this.$toast({text: '添加成功!', type: 's'})
        })
      }
    },
    getSocailContact () {
