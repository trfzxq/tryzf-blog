    ...
    <form class="form-inline">
      <div class="form-group">
        <div class="input-group">
          ...
          <input class="input" type="text" placeholder="text" v-model="navList.text"/>
        </div>
      </div>
      <div class="form-group">
        <div class="input-group">
          <div class="input-group-addon"><i class="glyphicon glyphicon-road"></i></div>
          <input class="input" type="text" placeholder="path" v-model="navList.path"/>
        </div>
      </div>
      <div class="form-group">
  ...
  methods: {
    removeNav (index) {
      ...
      this.$modal({
        ...
        text: '将永远删除，是否继续？',
        sureFn: function () {
          vm.$store.dispatch('removeNav', index)
        }
      })
    },
    addNav () {
      ...
      if (!data.path || !data.text) {
        return this.$toast({text: '内容不能空!', type: 'e'})
      }
